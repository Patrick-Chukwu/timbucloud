...
const Product = ({ product }) => {
  ...
  return (
    <motion.div whileHover={{opacity: .9}} className=" cursor-pointer flex flex-col gap-2 items-start justify-center bg-background shadow-lg rounded-[14px] xl:rounded-[20px] px-4 py-8  h-[480px] w-[335px] md:w-[312px] xl:w-[400px]  hover:shadow-xl" >
      ...
      <h2 className="text-xl font-bold text-foreground w-full  text-left" onClick={handleClick}>{product.name}</h2>
      ...
      <div className="flex gap-2 justify-between items-center w-full">
        ...
        <Button id="addToCart" className='text-xl bg-primary font-bold rounded-3xl hover:text-background hover:bg-destructive' onClick={handleAddToCart}>Add To Cart</Button>
      </div>
    </motion.div>
  );
};
